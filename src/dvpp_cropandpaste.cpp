...
DvppCropAndPaste::~DvppCropAndPaste() { DestroyCropAndPasteResource(); }
...
Result DvppCropAndPaste::InitCropAndPasteInputDesc(ImageData1 &inputImage) {
  ...
  if (alignWidth == 0 || alignHeight == 0) {
    ERROR_LOG("InitResizeInputDesc AlignmentHelper failed. image w %d, h %d, "
              "align w%d, h%d",
  ...
  acldvppSetPicDescData(vpcInputDesc_,
                        inputImage.data.get()); //  JpegD . vpcResize
...
#define SHARED_PRT_CROP_DVPP_BUF(buf)                                          \
  (shared_ptr<uint8_t>((uint8_t *)(buf), [](uint8_t *p) {                      \
    memcount--;                                                                \
    INFO_LOG("DvppCropAndPaste free RESIZEMALLOC memcount %d\n", memcount);    \
    acldvppFree(p);                                                            \
  }))

Result DvppCropAndPaste::Process(ImageData1 &resizedImage,
                                 ImageData1 &srcImage) {
  ...
  if (aclRet != ACL_ERROR_NONE) {
    ERROR_LOG("crop and paste aclrtSynchronizeStream failed, aclRet = %d",
              aclRet);
    return FAILED;
  }
