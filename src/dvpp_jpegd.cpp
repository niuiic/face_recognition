...
DvppJpegD::~DvppJpegD() { DestroyDecodeResource(); }
...
Result DvppJpegD::Process(ImageData1 &dest, ImageData1 &src) {
  ...
  aclError aclRet = acldvppJpegDecodeAsync(
      dvppChannelDesc_, reinterpret_cast<void *>(src.data.get()), src.size,
      decodeOutputDesc_, stream_);
  if (aclRet != ACL_ERROR_NONE) {
    ERROR_LOG("acldvppJpegDecodeAsync failed, error: %d", aclRet);
    return FAILED;
  }

  aclRet = aclrtSynchronizeStream(stream_);
  if (aclRet != ACL_ERROR_NONE) {
    ERROR_LOG("JpegD aclrtSynchronizeStream failed, error: %d", aclRet);
    return FAILED;
  }
